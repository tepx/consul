...
func (a *Agent) Start(ctx context.Context) error {
	...
	if a.config.AutoEncryptTLS && !a.config.ServerMode {
		...
		config := []certmon.Config{
			certmon.WithCache(a.cache),
			certmon.WithLogger(a.logger.Named(logging.AutoEncrypt)),
			certmon.WithTLSConfigurator(a.tlsConfigurator),
			certmon.WithTokens(a.tokens),
			certmon.WithFallback(a.autoEncryptInitialCertificate),
			certmon.WithInitialCerts(reply),
			certmon.WithDNSSANs(a.config.AutoEncryptDNSSAN),
			certmon.WithIPSANs(a.config.AutoEncryptIPSAN),
			certmon.WithDatacenter(a.config.Datacenter),
			certmon.WithNodeName(a.config.NodeName),
		}

		monitor, err := certmon.New(config...)
		if err != nil {
		...
		// to the agents lifetime by using the StopCh
		if err := a.certMonitor.Start(&lib.StopChannelContext{StopCh: a.shutdownCh}); err != nil {
